    ...
    <div className="workspace-shell">
      <header className="topbar">
        <div className="brand-lockup">
          ...
          <div>
            <p className="brand-name">Embedded Engineering Agent</p>
            <p className="brand-subtitle">Architecture Freeze · M21 Workbench</p>
          </div>
        </div>
        <div className="topbar-context">
          <label className="project-picker">
            <span>Project</span>
            <select aria-label="Current project" value={projectId ?? ""} onChange={(event) => setProjectId(event.target.value || null)}>
              <option value="">No project selected</option>
              {projects.map((project) => <option key={project.id} value={project.id}>{projectLabel(project)}</option>)}
            </select>
          </label>
          <span className="status-pill tone-pass">● Backend authenticated</span>
        </div>
      </header>

      <div className="workspace-body">
        <aside className="sidebar" aria-label="Primary navigation">
          <div className="sidebar-heading"><span>WORKSPACE</span><span className="project-chip">{selectedProject ? "PROJECT" : "START"}</span></div>
          <nav className="nav-list">
            {filteredNav.map((item) => <NavButton key={item.id} item={item} active={route === item.route} onNavigate={navigate} />)}
          </nav>
          <div className="sidebar-footer">
            <button className="text-button" onClick={() => setShowCreate(true)}>＋ New project</button>
        ...
        <main className="main-workspace">
          <div className="workspace-toolbar">
            <label className="search-box"><span aria-hidden="true">⌕</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search workspace" aria-label="Search workspace" /></label>
            <div className="toolbar-actions"><button className="ghost-button" onClick={() => projectId && void refreshProject(projectId)}>Refresh state</button><button className="primary-button" disabled={!projectId || Boolean(busy)} onClick={() => void runFullBenchmark()}>Run M20 UI workflow</button></div>
          </div>
          {error && <div className="feedback-banner feedback-error" role="alert"><strong>Backend action failed</strong><span>{error}</span><button className="icon-button" onClick={() => setError(null)} aria-label="Dismiss error">×</button></div>}
          {notice && <div className="feedback-banner feedback-success" role="status"><strong>Action complete</strong><span>{notice}</span><button className="icon-button" onClick={() => setNotice(null)} aria-label="Dismiss notice">×</button></div>}
          {busy && <div className="running-strip" role="status"><span className="spinner" aria-hidden="true" /> {busy} · deterministic backend operation running</div>}
        ...
        <aside className="context-panel" aria-label="Context and AI panel">
          <div className="panel-heading"><span className="panel-kicker">CONTEXT</span><span className="ai-spark">✦</span></div>
          <h2>{selectedProject?.name ?? "No project context"}</h2>
          <p className="muted">Backend state is authoritative. This panel explains state and suggests next actions; it never overrides deterministic gates.</p>
          {selectedProject && <div className="context-stack"><ContextRow label="Project ID" value={shortId(selectedProject.id)} /><ContextRow label="SourceRevision" value={shortId(context.source?.source_revision_id ?? workflow.firmware?.source_revision_id)} /><ContextRow label="Active domains" value={`${context.domains.filter((item) => item.status === "ACTIVE").length}`} /><ContextRow label="Open issues" value={displayCount(context.issues.filter((item) => item.status === "OPEN").length)} /></div>}
          <div className="ai-card"><div className="ai-card-title">AI Panel · controlled</div><p>Ask for a requirement interpretation or an explanation of a rule. Deterministic Build/ERC/Review states cannot be changed here.</p><button className="ghost-button full-width" onClick={() => navigate("ai")}>Open AI Panel</button></div>
          {selectedProject && <div className="context-actions"><button className="ghost-button full-width" onClick={() => navigate("review")}>Inspect release gate</button><button className="ghost-button full-width" onClick={() => setRawContext((value) => !value)}>{rawContext ? "Hide raw context" : "Show raw context"}</button></div>}
          {rawContext && <pre className="raw-json">{JSON.stringify({ project: selectedProject, context, workflow }, null, 2)}</pre>}
        </aside>
      </div>
      <footer className="statusbar"><span><i className="status-dot small pass" /> Backend connection: authenticated loopback</span><span>Source: {shortId(context.source?.source_revision_id ?? workflow.firmware?.source_revision_id)}</span><span>Build: <StatusPill status={asStatus(workflow.build?.status ?? context.latestBuild?.status)} /></span><span>Review: <StatusPill status={asStatus(workflow.review?.status ?? context.latestReview?.status)} /></span><span>{typeof context.consistency?.status === "string" ? `Consistency: ${statusLabel(context.consistency.status)}` : "State sync ready"}</span></footer>
...
function NavButton({ item, active, onNavigate }: { item: NavigationItem; active: boolean; onNavigate: (route: string) => void }) {
  return <button className={`nav-item ${active ? "active" : ""}`} aria-current={active ? "page" : undefined} onClick={() => onNavigate(item.route)}><span className="nav-icon" aria-hidden="true">{item.icon}</span><span>{item.label}</span>{item.extension && <span className="extension-badge">EXT</span>}</button>;
}

function StartPanel({ onCreate, onOpen }: { onCreate: () => void; onOpen: () => void }) {
  return <section className="start-panel"><span className="eyebrow">M21 · DESKTOP ENGINEERING WORKBENCH</span><h1>Bring verified engineering state into view.</h1><p>Create or open a project to work through requirements, verified pins, HardwareIR, MCUConfigIR, FirmwareIR, deterministic Build/Test/Review, and dynamic Domain UI.</p><div className="start-actions"><button className="primary-button" onClick={onCreate}>Create project</button><button className="ghost-button" onClick={onOpen}>Open project list</button></div><div className="start-contract"><span>✓ Backend/Core remains authority</span><span>✓ No domain is active by default</span><span>✓ UNKNOWN and STALE stay explicit</span></div></section>;
}
...
function PageFrame({ eyebrow, title, description, actions, children }: { eyebrow: string; title: string; description: string; actions?: React.ReactNode; children: React.ReactNode }) {
  return <section className="page-frame"><div className="page-header"><div><span className="eyebrow">{eyebrow}</span><h1>{title}</h1><p>{description}</p></div>{actions && <div className="page-actions">{actions}</div>}</div>{children}</section>;
}
...
function DashboardPage(props: PageProps) {
  ...
  return <PageFrame eyebrow="PROJECT / DASHBOARD" title={selectedProject?.name ?? "Dashboard"} description="A dense, answer-first view of the current engineering workflow and deterministic release health." actions={<button className="primary-button" disabled={Boolean(busy)} onClick={() => void onReview()}>Run review</button>}>
...
function PanelTitle({ label, title, action }: { label: string; title: string; action?: React.ReactNode }) {
  return <div className="panel-title"><div><span className="panel-kicker">{label}</span><h2>{title}</h2></div>{action}</div>;
}

function ActionRow({ title, detail, onClick, disabled }: { title: string; detail: string; onClick: () => Promise<void>; disabled?: boolean }) {
  return <button className="action-row" disabled={disabled} onClick={() => void onClick()}><span><strong>{title}</strong><small>{detail}</small></span><span className="chevron">›</span></button>;
}
...
function KeyValueList({ values }: { values: Record<string, string> }) {
  return <dl className="key-value-list">{Object.entries(values).map(([key, value]) => <div key={key}><dt>{key}</dt><dd title={value}>{value}</dd></div>)}</dl>;
}
...
function EmptyState({ text }: { text: string }) {
  return <div className="empty-state"><span className="empty-icon">○</span><span>{text}</span></div>;
}

function CopyValue({ value, label }: { value: string; label: string }) {
  const copy = async () => { await navigator.clipboard?.writeText(value); };
  return <button className="ghost-button" onClick={() => void copy()}>{label}</button>;
}
...
function CreateProjectDialog({ name, description, setName, setDescription, onClose, onCreate, busy }: { name: string; description: string; setName: (value: string) => void; setDescription: (value: string) => void; onClose: () => void; onCreate: () => void; busy: boolean }) {
  return <div className="dialog-backdrop" role="presentation"><section className="dialog" role="dialog" aria-modal="true" aria-labelledby="create-project-title"><div className="panel-title"><div><span className="panel-kicker">PROJECT</span><h2 id="create-project-title">Create project</h2></div><button className="icon-button" onClick={onClose} aria-label="Close dialog">×</button></div><label className="field-label" htmlFor="project-name">Name</label><input id="project-name" value={name} onChange={(event) => setName(event.target.value)} /><label className="field-label" htmlFor="project-description">Description</label><textarea id="project-description" value={description} onChange={(event) => setDescription(event.target.value)} rows={4} /><div className="dialog-actions"><button className="ghost-button" onClick={onClose}>Cancel</button><button className="primary-button" disabled={busy || !name.trim()} onClick={onCreate}>{busy ? "Creating…" : "Create project"}</button></div></section></div>;
}
